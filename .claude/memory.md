...
| **Local Admin CMS (`/admin`) & File Persistence** | Provides a visual GUI on local environment to CRUD portfolio data. Saves changes directly to `portfolioData.json` using Node.js `fs/promises`. Deploying to production simply requires `git commit` and `git push`. |
